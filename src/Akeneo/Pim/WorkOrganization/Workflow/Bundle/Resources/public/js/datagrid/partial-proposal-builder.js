...
define(['jquery', 'underscore', 'pim/form-modal', 'routing', 'oro/messenger', 'oro/mediator'], function(
  $,
  _,
  FormModal,
  Routing,
  ...
  return {
    ...
    init: function(gridElement, gridName) {
      gridElement.on(
        'click',
        '.partial-approve-link, .partial-reject-link',
        function(event) {
    ...
    doAction: function(action, routing, gridName) {
      return $.post(routing)
        .then(() => {
          messenger.notify('success', _.__('pimee_workflow.proposal.partial_' + action + '.modal.success'));
        ...
        .fail(() => {
          messenger.notify('error', _.__('pimee_workflow.proposal.partial_' + action + '.modal.error'));
        });
    },
  };
});
