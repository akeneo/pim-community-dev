...
class ApiFindNewAnnouncementIdsIntegration extends KernelTestCase
{
    ...
    {
        $query = self::$kernel->getContainer()->get('akeneo_communication_channel.query.api.find_new_announcement_ids');
    ...
    public function waitServerUp()
    {
        ...
        do {
            try {
                $httpClient = new Client(['base_uri' => self::$kernel->getContainer()->getParameter('comm_panel_api_url')]);
                $httpClient->get('/');
