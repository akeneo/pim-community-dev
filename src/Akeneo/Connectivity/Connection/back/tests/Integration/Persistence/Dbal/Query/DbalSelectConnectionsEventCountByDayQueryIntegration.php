...
class DbalSelectConnectionsEventCountByDayQueryIntegration extends TestCase
{
    ...
    public function test_it_get_data_for_connections()
    {
        ...
        $this->auditLoader->insertData('<all>', new \DateTime('2020-01-02'), 10, EventTypes::PRODUCT_UPDATED);
        $this->auditLoader->insertData('<all>', new \DateTime('2020-01-04'), 0, EventTypes::PRODUCT_UPDATED);

        $weeklyEventCountsPerConnection = $this->selectConnectionsEventCountByDayQuery->execute(EventTypes::PRODUCT_UPDATED, date('2020-01-01'), date('2020-01-08'));
        $result = array_reduce(
