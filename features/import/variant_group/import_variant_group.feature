...
Feature: Execute an import
  ...
  Scenario: Import is stopped when we try to change axis
    Given the following CSV file to import:
    """
    code;axis;label-en_US
    SANDAL;size;"Sandal"
  ...
  Scenario: Import is stopped when we try to update a group which is not a variant group
    Given the following CSV file to import:
    """
    code;axis;label-en_US
    SANDAL;size;"Sandal"
    ...
    And I launch the import job
    And I wait for the "footwear_variant_group_values_import" job to finish
    Then there should be 2 variant groups
    And I should see "Read 1"
    And I should see "Updated 1"