...
const MappedFilterDropdown = ({value, onChange}: MappedFilterDropdownProps) => {
  ...
  return (
    <Dropdown>
      ...
      >
        {translate(
          `akeneo.syndication.data_mapping_details.sources.operation.replacement.modal.filters.mapped.${value}`
        )}
      </SwitcherButton>
      {isDropdownOpen && (
        <Dropdown.Overlay verticalPosition="down" onClose={close}>
          <Dropdown.ItemCollection>
            {mappedFilterValues.map(filterValue => (
              ...
              >
                {translate(
                  `akeneo.syndication.data_mapping_details.sources.operation.replacement.modal.filters.mapped.${filterValue}`
                )}
              </Dropdown.Item>
            ))}
          </Dropdown.ItemCollection>
