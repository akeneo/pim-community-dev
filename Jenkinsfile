...
node {
    ...
    sh "/usr/bin/php7.0 /var/lib/distributed-ci/dci-master/bin/build -p ${userInput['php_version']} -m ${userInput['mysql_version']} ${env.WORKSPACE} ${env.BUILD_NUMBER} pim-community-dev ${userInput['storage']} ${userInput['features']} akeneo/job/pim-community-dev/view/Pull%20Requests/job/${env.JOB_BASE_NAME} ${userInput['attempts']}"
}
