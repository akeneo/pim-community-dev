...
final class SqlGetGroupedAttributesIntegration extends TestCase
{
    ...
    {
        $results = $this->getSqlGetGroupedAttributes()->findAttributes('en_US', 100);

        $this->assertNotNull($this->findAttributeInResults('erp', 'erp_name', $results));
        $this->assertNotNull($this->findAttributeInResults('marketing', 'name', $results));
        $this->assertNotNull($this->findAttributeInResults('marketing', 'description', $results));

        $results = $this->getSqlGetGroupedAttributes()->findAttributes('en_US', 100, null, 0, 'descr');
    ...
    {
        $results = $this->getSqlGetGroupedAttributes()->findAttributes('en_US', 4, null, 1);
        $this->assertCount(4, $results);
        $codeForFirstResult = $results[0]->code;

        $results = $this->getSqlGetGroupedAttributes()->findAttributes('en_US', 4, null, 2);
        $this->assertCount(4, $results);
        $this->assertNotEquals($codeForFirstResult, $results[0]->code);

        $results = $this->getSqlGetGroupedAttributes()->findAttributes('en_US', 4, null, 200);
        $this->assertCount(0, $results);
    }

    public function test_it_uses_the_locale_code_for_labels(): void
    {
        $results = $this->getSqlGetGroupedAttributes()->findAttributes('fr_FR', 100);
        ...
        $results = $this->getSqlGetGroupedAttributes()->findAttributes('unnown', 100);
        $this->assertNotEmpty($results);
        $erpNameAttribute = $this->findAttributeInResults('erp', 'erp_name', $results);
        $this->assertNotNull($erpNameAttribute);
    ...
    /**
     * @param GroupedAttribute[] $results
     */
    private function findAttributeInResults(string $attributeGroupCode, string $attributeCode, array $results): ?GroupedAttribute
    {
    ...
    {
        return $this->get('akeneo.pim.structure.query.get_grouped_attributes');
    }
