...
class Creation extends Form
{
    ...
    public function __construct($session, $pageFactory, $parameters = [])
    {
        ...
        $this->elements = array_merge(
            $this->elements,
            [
                'attribute_option_table' => ['css' => sprintf('tr[role=%s]', 'attribute-option-list')],
                'attribute_options'      => ['css' => sprintf('tr[role=%s]', 'attribute-option-item')],
    ...
    public function checkOptionsOrder(array $expectedOrder)
    {
        $this->spin(function () use ($expectedOrder) {
            ...
            $actualOrder = array_map(function ($row) {
                $option = $row->find('css', sprintf('*[role=%s]', 'attribute-option-item-code'));
    ...
    protected function getOptionElement($optionName)
    {
        foreach ($this->getOptionsElement() as $optionRow) {
            if ($optionRow->find('css', sprintf('*[role=%s]', 'attribute-option-item-code')) &&
                strtolower($optionRow->find('css', sprintf('*[role=%s]', 'attribute-option-item-code'))->getText()) === strtolower($optionName)
            ) {
                return $optionRow;
            }
        }
