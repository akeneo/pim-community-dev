...
const ProposalDiffMeasurement: React.FC<ProposalDiffMeasurementProps> = ({accessor, change, ...rest}) => {
  ...
  return (
    <span {...rest}>
      ...
      &nbsp;
      {isUnitDiff && accessor === 'before' && <del>{beforeData.unit}</del>}
      {isUnitDiff && accessor === 'after' && <ins>{afterData.unit}</ins>}
      {!isUnitDiff && accessor === 'before' && beforeData.unit}
      {!isUnitDiff && accessor === 'after' && afterData.unit}
    </span>
  );
};
