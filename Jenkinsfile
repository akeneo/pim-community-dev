...
stage("Integration tests") {
    ...
    tasks["phpunit-integration-5.6"] = {runPhpUnitIntegrationTest("5.6")}
    tasks["phpunit-integration-7.0"] = {runPhpUnitIntegrationTest("7.0")}
...
def runPhpUnitIntegrationTest(version) {
    node('docker') {
        deleteDir()
        try {
...
def runBehatTest(edition, storage, features, phpVersion, mysqlVersion, esVersion) {
    node() {
        dir("behat-${edition}-${storage}") {
            ...
            try {
                sh "php /var/lib/distributed-ci/dci-master/bin/build ${env.WORKSPACE}/behat-${edition}-${storage} ${env.BUILD_NUMBER} ${storage} ${features} ${env.JOB_NAME} 5 ${phpVersion} ${mysqlVersion} \"${tags}\" \"behat-${edition}-${storage}\" -e ${esVersion}"
            } finally {
