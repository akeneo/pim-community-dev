...
define(['jquery', 'underscore', 'pim/form-registry', 'require-context'], function ($, _, FormRegistry, requireContext) {
...
Check your form_extension files and be sure to clear your prod cache before proceeding
  ...
  const buildForm = function (formMeta) {
    return FormRegistry.getFormExtensions(formMeta).then(extensionsMeta => {
      ...
      const filteredExtensionsMeta = extensionsMeta.filter(extensionsMeta => null !== extensionsMeta.module);
