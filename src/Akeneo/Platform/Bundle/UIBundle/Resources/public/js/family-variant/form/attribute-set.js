    ...
    return Object.values(attributeGroups)
      ...
      .map(attributeGroup => {
        const groupAttributes = attributes
          .filter(attribute => attribute.group === attributeGroup.code && attributeCodes.indexOf(attribute.code) !== -1)
          .sort(sortOrdered);

        const locked = groupAttributes.filter(attribute => !lockedAttributes.includes(attribute.code)).length === 0;
  ...
  return BaseForm.extend({
    ...
    render: function () {
      ...
      fetcherRegistry
        ...
        .then(family => {
          const axesAttributeCodes = familyVariant.variant_attribute_sets.reduce(
        ...
        .then((attributeGroups, attributes, axesAttributeCodes, family) => {
          ...
          const axesAttributes = familyVariant.variant_attribute_sets
            .map(set => set.axes)
    ...
    moveAttribute(lockedAttributes) {
      return (event, ui) => {
        ...
        const movedAttributes = [ui.item[0].dataset.attributeCode].filter(
          movedAttribute => !lockedAttributes.includes(movedAttribute)
        );

        this.handleAttributesMove(originLevel, destinationLevel, movedAttributes);
      };
    },
    ...
    moveAttributes(lockedAttributes) {
      return (event, ui) => {
        const destinationLevel = parseInt(ui.item[0].parentNode.dataset.level);
        const originLevel = parseInt(ui.sender[0].dataset.level);
        const movedAttributes = Object.values(ui.item[0].querySelectorAll('li'))
          .map(domElement => domElement.dataset.attributeCode)
    ...
    removeAttributeGroupFromVariantAttributeSet(event) {
      ...
      const removedAttributes = $attributeGroupToRemove
        .find('.attribute.deletable')
        .toArray()
