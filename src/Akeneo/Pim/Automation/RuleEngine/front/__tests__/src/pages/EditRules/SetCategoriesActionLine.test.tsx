...
describe('SetCategoriesActionLine', () => {
  ...
  it('should display the set categories action line, and switch tree', async () => {
    ...
    expect(
      await findByTestId('category-tree-selector-master')
    ).toBeInTheDocument();
    expect(
      await findByTestId('category-tree-selector-sales')
    ).toBeInTheDocument();
    expect(
      queryByTestId('category-tree-selector-marketing')
    ).not.toBeInTheDocument(); // As there are no categories with this tree
    expect(
      await findByTestId('category-tree-selector-new')
    ).toBeInTheDocument();
    expect(
      (await findByTestId('category-tree-selector-new')).children.length
  ...
  it('should display the unknown categories', async () => {
    ...
    expect(
      await findByText('pimee_catalog_rule.exceptions.unknown_categories')
    ).toBeInTheDocument();
  });
});
