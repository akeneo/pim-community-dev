...
<Canvas>
  <Story name="Standard">
    {args => {
      const {getValue, setValue, searchValue, setSearchValue, match, weightUnits} = useFakeTableInput(6);
      return <TableInput {...args}>
        <TableInput.Header>
          ...
          <TableInput.HeaderCell>Weight</TableInput.HeaderCell>
          ...
          {['Sugar', 'Salt', 'Pepper', 'Eggs', 'Flour', 'Yeast'].map((row, index) => <TableInput.Row key={row}>
            ...
            <TableInput.Cell>
              <TableInput.Measurement
                amount={getValue(index, 'weight').amount}
                unit={getValue(index, 'weight').unit}
                units={weightUnits}
                onChange={(amount, unit) => setValue(index, 'weight', {unit, amount})}
              />
            </TableInput.Cell>
            <TableInput.Cell>
              <TableInput.Select
