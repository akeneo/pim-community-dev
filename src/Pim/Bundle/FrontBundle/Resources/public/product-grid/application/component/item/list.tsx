...
export default class ListElement extends React.Component<Props, State> {
  ...
  render(): JSX.Element | JSX.Element[] {
    ...
    const clickAction = () => {
      product instanceof ProductModel && product.shouldHaveChildren()
        ? onLoadChildren(product)
    ...
    const row = (
      <tr
        className={`AknGrid-bodyRow row-click-action AknGrid-bodyRow--depth${depth} ${
          this.state.isDisplayed ? 'AknGrid-bodyRow--displayed' : 'AknGrid-bodyRow--hidden'
        }`}
        data-identifier={product.getIdentifier()}
        onClick={clickAction}
        style={{transitionDelay: `${position * 10}ms`}}
      >
        ...
        <td className="AknGrid-bodyCell AknGrid-bodyCell--highlight" data-column="label">
          {product.getLabel(channel, locale)}
        </td>
        <td className="AknGrid-bodyCell string-cell" data-column="family">
          {product.family}
        </td>
        <td className="AknGrid-bodyCell string-cell">
          <div className="AknBadge AknBadge--medium AknBadge--disabled status-disabled">
            <i className="AknBadge-icon icon-status-disabled icon-circle" />Disabled
          </div>
        </td>
        <td className="AknGrid-bodyCell string-cell">
        ...
        <td className="AknGrid-bodyCell string-cell" data-column="created">
          01/05/2018
        </td>
        <td className="AknGrid-bodyCell string-cell" data-column="updated">
          01/09/2018
        </td>
