...
Background:
    ...
    Scenario: Import a calculate rule with non numeric attribute types
        ...
        And an exception with message "actions[0].destination.field: Invalid attribute type for \"name\", expected a number attribute" has been thrown
        And an exception with message "actions[0].source.field: Invalid attribute type for \"description\", expected a number attribute" has been thrown
        And an exception with message "actions[0].operation_list[0].field: Invalid attribute type for \"color\", expected a number attribute" has been thrown
