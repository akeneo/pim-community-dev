...
const AttributesListWithVariations = ({
  product,
  criterionCode,
  attributes,
  locale,
  axis,
  evaluation,
}: AttributesListWithVariationsProps) => {
  ...
  return isRootProductModel(product) ? (
    <AttributesList
      product={product}
      ...
      {product.meta.variant_navigation
        ...
        .map((variant: any, level: number) => {
          return (
            ...
            >
              <span>
                {level > 0 ? variant.axes[locale] : __('pim_enrich.entity.product.module.variant_navigation.common')}
              </span>
              :&thinsp;
              <AttributesList
                product={product}
