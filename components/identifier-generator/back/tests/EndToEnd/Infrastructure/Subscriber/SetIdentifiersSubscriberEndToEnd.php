...
class SetIdentifiersSubscriberEndToEnd extends EndToEndTestCase
{
    ...
    /** @test */
    public function it_should_generate_the_next_identifier_if_there_is_already_one_created_manually(): void
    {
        $this->createIdentifierGenerator();
        $firstAutoGeneratedProduct = $this->createProduct();
        Assert::assertSame('akn-050-my_family', $firstAutoGeneratedProduct->getValue('sku')->getData());

        $this->createProduct('akn-051-my_family');
        $this->createProduct('akn-052-my_family');

        $secondAutoGeneratedProduct = $this->createProduct();
        Assert::assertNotNull($secondAutoGeneratedProduct->getValue('sku'));
        Assert::assertSame('akn-053-my_family', $secondAutoGeneratedProduct->getValue('sku')->getData());
    }
