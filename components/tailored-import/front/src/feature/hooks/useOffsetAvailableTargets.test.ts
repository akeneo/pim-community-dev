...
import {TargetOffset} from 'feature/models';
import {useOffsetAvailableTargets} from './useOffsetAvailableTargets';
...
const mockedFetcher = (offset: TargetOffset) => {
...
jest.mock('./useAvailableTargetsFetcher', () => ({
  useAvailableTargetsFetcher: () => mockedFetcher,
}));

const flushPromises = () => new Promise(setImmediate);

test('It can offset available target results', async () => {
  const {result} = renderHook(() => useOffsetAvailableTargets('search value'));
...
test('It does not update results if unmounted', async () => {
  const {result, unmount} = renderHook(() => useOffsetAvailableTargets('search value'));
...
test('It does not update results if the shouldFetch param is set to false', async () => {
  const {result} = renderHook(() => useOffsetAvailableTargets('search value', false));
...
test('It goes back to first page when search value changes', async () => {
  const {result, rerender} = renderHook(({searchValue}) => useOffsetAvailableTargets(searchValue), {
