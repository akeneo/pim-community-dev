...
class IdentifierValueSetterSpec extends ObjectBehavior
{
    function let(ProductBuilder $builder)
    {
        $this->beConstructedWith($builder, ['pim_catalog_identifier']);
    }
    ...
    function it_supports_identifier_attributes(
        AttributeInterface $identifierAttribute,
        AttributeInterface $textareaAttribute
    ) {
        $identifierAttribute->getAttributeType()->willReturn('pim_catalog_identifier');
        $this->supports($identifierAttribute)->shouldReturn(true);
    ...
    {
        $this->getSupportedTypes()->shouldReturn(['pim_catalog_identifier']);
    }

    function it_throws_an_error_if_data_is_not_a_identifier(
        ...
        $data = 'not a identifier';

        $this->shouldThrow(
            new \LogicException('Attribute "attributeCode" expects a identifier as data')
        )->during('setValue', [[], $attribute, $data, 'fr_FR', 'mobile']);
    }

    function it_sets_identifier_value_to_a_product_value(
        AttributeInterface $attribute,
