...
const selectedColumn = getComplexTableAttribute().table_configuration.find(columnDefinition => columnDefinition.data_type === 'number') as NumberColumnDefinition;

describe('NumberProperties', () => {
  it('should render the component', () => {
    renderWithProviders(
      <NumberProperties attribute={getComplexTableAttribute()} selectedColumn={selectedColumn} handleChange={jest.fn()}/>
    );
  ...
  it('should callback changes', () => {
    ...
    renderWithProviders(
      <NumberProperties attribute={getComplexTableAttribute()} selectedColumn={selectedColumn} handleChange={handleChange}/>
    );
    ...
    const decimalsAllowedCheckbox = screen.getByLabelText('pim_table_attribute.validations.decimals_allowed') as HTMLInputElement;
  ...
  it('should display error message when min is greater than max', () => {
    ...
    renderWithProviders(
      <NumberProperties attribute={getComplexTableAttribute()} selectedColumn={erroredSelectedColumn} handleChange={jest.fn()}/>
    );
  ...
  it('should unset validations when values are not filled anymore', () => {
    const filledSelectedColumn = {...selectedColumn, validations: {
        min: 50,
        max: 10,
        decimals_allowed: true,
      }};
    const handleChange = jest.fn();
    renderWithProviders(
      <NumberProperties attribute={getComplexTableAttribute()} selectedColumn={filledSelectedColumn} handleChange={handleChange}/>
    );
