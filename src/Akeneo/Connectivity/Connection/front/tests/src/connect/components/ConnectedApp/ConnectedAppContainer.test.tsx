...
test('The Open App button is disabled when the user doesnt have the permission to Open Apps', async () => {
    ...
    const openAppButton = expect(screen.queryByText('pim_menu.item.open_app'));
...
test('The Open App button is in warning state for test app when the connectedApp has a outdated scopes flag', async () => {
    ...
    expect(screen.queryByText('pim_menu.item.open_app')).toHaveStyle('background-color: rgb(249, 181, 63)');
});
...
const assertPageHeader = () => {
    ...
    expect(screen.queryByText('pim_menu.item.open_app')).toBeInTheDocument();
};
