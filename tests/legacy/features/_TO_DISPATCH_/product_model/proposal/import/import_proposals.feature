Feature: Import product model proposals
  ...
  Scenario: Update a product model proposal
    Given the following CSV file to import:
      """
      code;name-fr_FR
      my-jacket;My jacket
    ...
    Given the following CSV file to import:
      """
      code;description-fr_FR-mobile;description-fr_FR-ecommerce
      my-jacket;Ma desc;Ma description
      """
    When Mary imports "product model proposal" via the job csv_product_model_proposal_import
    Then the proposal for product model "my-jacket" and author Mary should be:
      | field                       | value          |
  ...
  Scenario: Remove a product model proposal if there is no diff
    Given the following CSV file to import:
      """
      code;handmade
      my-jacket;0
      """
    When Mary imports "product model proposal" via the job csv_product_model_proposal_import
    Then there is one proposal for product model "my-jacket" and author Mary
    Given the following CSV file to import:
      """
      code;handmade
      my-jacket;
      """
    When Mary imports "product model proposal" via the job csv_product_model_proposal_import
    Then there is no proposal for product model "my-jacket" and author Mary
