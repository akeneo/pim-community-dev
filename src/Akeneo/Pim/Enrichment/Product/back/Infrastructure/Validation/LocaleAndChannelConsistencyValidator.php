...
final class LocaleAndChannelConsistencyValidator extends ConstraintValidator
{
    ...
    private function validateChannelCode(Attribute $attribute, ?string $channelCode): void
    {
        if (!$attribute->isScopable()) {
            if (null !== $channelCode) {
                $this->addViolation(LocaleAndChannelConsistency::CHANNEL_CODE_PROVIDED_FOR_NON_SCOPABLE_ATTRIBUTE);
            }
        ...
        if (null === $channelCode) {
            $this->addViolation(LocaleAndChannelConsistency::NO_CHANNEL_CODE_PROVIDED_FOR_SCOPABLE_ATTRIBUTE);

            return;
        }
        if (!$this->channelExistsWithLocale->doesChannelExist($channelCode)) {
            $this->addViolation(LocaleAndChannelConsistency::CHANNEL_DOES_NOT_EXIST);
        }
    }
    ...
    private function validateLocaleCode(Attribute $attribute, ?string $localeCode, ?string $channelCode): void
    {
        if (!$attribute->isLocalizable()) {
            if (null !== $localeCode) {
                $this->addViolation(LocaleAndChannelConsistency::LOCALE_CODE_PROVIDED_FOR_NON_LOCALIZABLE_ATTRIBUTE);
            }
        ...
        if (null === $localeCode) {
            $this->addViolation(LocaleAndChannelConsistency::NO_LOCALE_CODE_PROVIDED_FOR_LOCALIZABLE_ATTRIBUTE);

            return;
        }

        if (!$attribute->isScopable() && !$this->channelExistsWithLocale->isLocaleActive($localeCode)) {
            $this->addViolation(LocaleAndChannelConsistency::LOCALE_IS_NOT_ACTIVE);
        ...
        if ($attribute->isScopable() && null !== $channelCode &&
            ...
            $this->addViolation(LocaleAndChannelConsistency::LOCALE_NOT_BOUND_TO_CHANNEL);

            return;
        }

        if ($attribute->isLocaleSpecific() && !\in_array($localeCode, $attribute->availableLocaleCodes())) {
            $this->addViolation(LocaleAndChannelConsistency::INVALID_LOCALE_CODE_FOR_LOCALE_SPECIFIC_ATTRIBUTE);
        }
    }
