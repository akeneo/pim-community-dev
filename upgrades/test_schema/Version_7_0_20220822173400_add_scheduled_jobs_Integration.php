...
final class Version_7_0_20220822173400_add_scheduled_jobs_Integration extends TestCase
{
    use ExecuteMigrationTrait;

    private const MIGRATION_LABEL = '_7_0_20220822173400_add_scheduled_jobs';
    ...
    {
        $this->deleteJobInstance('versioning_purge');
        $this->assertNull($this->jobInstanceId('versioning_purge'));
        $this->reExecuteMigration(self::MIGRATION_LABEL);
        $this->assertNotNull($this->jobInstanceId('versioning_purge'));
    }

    public function test_it_does_not_adds_an_instance_if_present()
    {
        $jobInstanceId = $this->jobInstanceId('versioning_purge');
        $this->assertNotNull($jobInstanceId);
        $this->reExecuteMigration(self::MIGRATION_LABEL);
        $this->assertEquals($jobInstanceId, $this->jobInstanceId('versioning_purge'));
    }
