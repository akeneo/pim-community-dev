...
const FileTemplateConfigurator = ({fileInfo, fileStructure, onFileStructureChange, validationErrors}: FileTemplateConfiguratorProps) => {
  ...
  const handleSheetChange = async (sheetName: string) => {
    if (fileTemplateInformation) {
      ...
      setFileTemplateInformation(
        await fileTemplateInformationFetcher(fileTemplateInformation.file_info, sheetName)
      );
    }
  };
