...
class GetAssociatedProductCodesByProductIntegration extends TestCase
{
    public function testQueryToGetAssociatedProductCodes()
    {
        ...
        $this->assertSame(['productView'], $query->getCodes(4, $xsell->getId()));
        $this->assertSame(['productWithoutCategory'], $query->getCodes(4, $pack->getId()));
        $this->assertSame([], $query->getCodes(4, $upsell->getId()));
    }
