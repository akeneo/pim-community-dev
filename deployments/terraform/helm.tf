...
data "template_file" "helm_pim_config" {
  ...
  vars = {
    ...
    bigcommerce_connector_enabled                            = local.type != "tria" ?true : false
    bigcommerce_connector_topic                              = local.type != "tria" ?google_pubsub_topic.connector_bigcommerce[0].name : "fakeValue"
