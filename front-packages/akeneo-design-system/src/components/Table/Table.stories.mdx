...
<Canvas>
  <Story name="Draggable rows">
    {() => {
      const tableRows = [...new Array(1000)].map((_, index2) => ({id: index2, name: `row ${index2}`}));
      const [orderedRows, setOrderedRow] = useState(tableRows);
      const handleOnReorder = useCallback(newIndices => {
        setOrderedRow(rows => newIndices.map(index => rows[index]));
      }, []);
      return (
        <Scrollable height={500}>
          <Table isDragAndDroppable={true} onReorder={handleOnReorder}>
            <Table.Header>
              <Table.HeaderCell>Name</Table.HeaderCell>
              <Table.HeaderCell>Actions</Table.HeaderCell>
            </Table.Header>
