...
<Meta
  ...
  argTypes={{
    SectionTitle: {control: {type: 'text'}, description: 'Define the section title text'},
  }}
  args={{
    SectionTitle: 'General parameters',
  }}
/>
...
<Canvas>
  <Story name="Standard">
    {args => {
      return (
        <SectionTitle>
          <SectionTitle.Title>{args.SectionTitle}</SectionTitle.Title>
        </SectionTitle>
      );
    }}
  </Story>
