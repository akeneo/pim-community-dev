...
import {SourceOffset} from '../../models';
...
test('It can offset available source results', async () => {
  const {result} = renderHook(() => useOffsetAvailableSources('search value', 'string'));
...
test('It does not update results if unmounted', async () => {
  const {result, unmount} = renderHook(() => useOffsetAvailableSources('search value', 'string'));
...
test('It does not update results if the shouldFetch param is set to false', async () => {
  const {result} = renderHook(() => useOffsetAvailableSources('search value', 'string', false));
...
test('It goes back to first page when search value changes', async () => {
  const {result, rerender} = renderHook(({searchValue}) => useOffsetAvailableSources(searchValue, 'string'), {
