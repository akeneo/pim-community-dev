...
const MeasurementInput: CellInput = ({columnDefinition, highlighted, inError, row, onChange}) => {
  ...
  const units = useMeasurementUnits((columnDefinition as MeasurementColumnDefinition).measurementFamilyCode);
  ...
  const handleChange = React.useCallback((amount: string | undefined, unit: string) => amount && onChange({amount, unit}), [onChange]);
