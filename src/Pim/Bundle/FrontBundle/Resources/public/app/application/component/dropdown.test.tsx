...
describe('>>>COMPONENT --- dropdown', () => {
  test('Display a simple dropdown with a label', () => {
    ...
    />);
    expect(dropdown.find('.AknDropdown-menuTitle').text().trim()).toEqual('my dropdown');
    expect(dropdown.find('.AknActionButton-highlight').text().trim()).toEqual('and this is the second item');
  });
  ...
  test('Opens on click, list options and change value on selection', () => {
    ...
    dropdown.find('.AknDropdown-menuLink').first().simulate('click');
    expect(dropdown.find('.AknDropdown-menu--open').length).toEqual(0);
    expect(dropdown.find('.AknActionButton-highlight').text().trim()).toEqual('actually this is the first item');
  });
  ...
  test('No update on value if same value', () => {
    ...
    dropdown.find('.AknActionButton').simulate('click');
    dropdown.find('.AknDropdown-menuLink--active').first().simulate('click');
    expect(value).toEqual('my_custom_value');
  });
