import {formatBackendRanksToVictoryFormat} from "@akeneo-pim-ee/data-quality-insights/src/application/helper/Dashboard/FormatBackendRanksToVictoryFormat";
...
describe('Dashboard convert backend ranks to Victory format', () => {
  ...
  test('No axis rates should return data for only the rank 6', () => {
    ...
    const rates0 = dates.map((date: string) => {return {x: date, y: 0}});
    const rates100 = dates.map((date: string) => {return {x: date, y: 100}});
