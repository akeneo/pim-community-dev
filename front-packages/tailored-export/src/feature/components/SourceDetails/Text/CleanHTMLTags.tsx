...
const CleanHTMLTags = ({operation = getDefaultCleanHTMLTagsOperation(), onOperationChange}: CleanHTMLTagsProps) => {
  ...
  return (
    <Collapse
      ...
      label={
        <>
          {translate('akeneo.tailored_export.column_details.sources.operation.clean_html_tags.title')}
    ...
    >
      ...
      >
        {translate('akeneo.tailored_export.column_details.sources.operation.clean_html_tags.label')}
      </Checkbox>
    </Collapse>
  );
};
