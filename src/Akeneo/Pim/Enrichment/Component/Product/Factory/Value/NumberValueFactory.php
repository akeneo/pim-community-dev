...
final class NumberValueFactory extends ScalarValueFactory implements ValueFactory
{
    public function createWithoutCheckingData(Attribute $attribute, ?string $channelCode, ?string $localeCode, $data): ValueInterface
    {
        ...
        if ($attribute->isLocalizableAndScopable()) {
            return new NumberValue($attributeCode, $data, $channelCode, $localeCode);
        }

        if ($attribute->isScopable()) {
            return new NumberValue($attributeCode, $data, $channelCode, null);
        }

        if ($attribute->isLocalizable()) {
            return new NumberValue($attributeCode, $data, null, $localeCode);
        }

        return new NumberValue($attributeCode, $data, null, null);
    }
