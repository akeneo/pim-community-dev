...
const getAssetCollectionAttribute = () => ({
  code: 'name',
...
test('it displays an asset collection configurator', async () => {
  const attribute = getAssetCollectionAttribute();
...
test('it throws an error if we setup this component with a wrong target', async () => {
  ...
  }).rejects.toThrow('Invalid target data "family" for asset collection configurator');
