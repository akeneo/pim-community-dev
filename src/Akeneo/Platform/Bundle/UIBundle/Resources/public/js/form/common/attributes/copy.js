...
define([
  ...
  return BaseForm.extend({
    ...
    configure: function () {
      ...
      this.getScopeLabel(this.scope).then(
        function (scopeLabel) {
      ...
      return this.getScopeLabel(this.scope)
        .then(
          function (scopeLabel) {
            this.scopeLabel = scopeLabel;
          }.bind(this)
    ...
    canBeCopied: function (field) {
      return (
        ...
        (!field.attribute.is_locale_specific || _.contains(field.attribute.available_locales, this.getLocale()))
      );
    },
    ...
    setScope: function (scopeCode) {
      this.getScopeLabel(scopeCode).then(
        function (scopeLabel) {
          this.scopeLabel = scopeLabel;
    ...
    selectAll: function () {
      ...
      _.each(
        this.getSourceData(),
        function (value, attributeCode) {
          fieldPromises.push(FieldManager.getField(attributeCode));
        }.bind(this)
      );

      $.when.apply(this, fieldPromises).then(
        function () {
