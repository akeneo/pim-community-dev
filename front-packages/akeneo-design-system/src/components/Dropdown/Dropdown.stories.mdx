...
<Canvas>
  <Story name="With search and pagination" height={260}>
    {args => {
      const [isOpen, open, close] = useBooleanState(true);
      const Overlay = () => {
        const [searchValue, setSearchValue] = useState('');
        const debouncedSearchValue = useDebounce(searchValue)
        const [items, handleNextPage] = usePaginatedResults((page) => fakeFetcher(page, debouncedSearchValue), [debouncedSearchValue]);
        return (<Dropdown.Overlay verticalPosition="down" onClose={close}>
          <Dropdown.Header>
            <Search
