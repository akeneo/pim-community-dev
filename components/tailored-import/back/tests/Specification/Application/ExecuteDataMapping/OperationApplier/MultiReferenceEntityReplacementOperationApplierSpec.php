...
use Akeneo\Platform\TailoredImport\Application\ExecuteDataMapping\OperationApplier\MultiReferenceEntityReplacementOperationApplier;
...
class MultiReferenceEntityReplacementOperationApplierSpec extends ObjectBehavior
{
    private string $uuid = '00000000-0000-0000-0000-000000000000';

    public function it_supports_multi_reference_entity_replacement_operation(): void
    {
        $this->supports(new MultiReferenceEntityReplacementOperation($this->uuid, [
            'adidas' => ['nike', 'reebok'],
            'foo' => ['bar', 'baz'],
        ]))->shouldReturn(true);
    }

    public function it_applies_multi_reference_entity_replacement_operation(): void
    {
        $operation = new MultiReferenceEntityReplacementOperation($this->uuid, [
            'adidas' => ['nike', 'reebok'],
            'foo' => ['bar', 'baz'],
            'int' => ['8'],
        ]);
    ...
    public function it_throws_an_exception_when_operation_type_is_invalid(): void
    {
        ...
        $this->shouldThrow(new UnexpectedValueException(
            $operation,
            MultiReferenceEntityReplacementOperation::class,
            MultiReferenceEntityReplacementOperationApplier::class,
