...
class SqlGetProductLabels implements GetProductLabelsInterface
{
    ...
    public function byCodesAndLocaleAndScope(array $identifiers, string $locale, string $channel): array
    {
        $query = <<<SQL
SELECT
    p.identifier,
    ...
    JSON_MERGE_PATCH(COALESCE(p.raw_values, '{}'), COALESCE(pm1.raw_values, '{}'), COALESCE(pm.raw_values, '{}')) as raw_values
