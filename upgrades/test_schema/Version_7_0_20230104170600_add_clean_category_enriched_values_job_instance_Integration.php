...
final class Version_7_0_20230104170600_add_clean_category_enriched_values_job_instance_Integration extends TestCase
{
    ...
    public function test_it_adds_the_job_instance(): void
    {
        $this->connection->executeStatement(
            <<<SQL
            DELETE FROM akeneo_batch_job_instance WHERE code = 'clean_category_enriched_values';
            SQL
        );
    ...
    private function jobInstanceExists(): bool
    {
        return (bool) $this->connection->fetchOne(
            <<<SQL
            SELECT EXISTS(
                SELECT * FROM akeneo_batch_job_instance WHERE code = 'clean_category_enriched_values'
            );
            SQL
        );
    }
