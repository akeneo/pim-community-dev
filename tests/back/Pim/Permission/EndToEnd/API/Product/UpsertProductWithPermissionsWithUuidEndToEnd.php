 ...
 * +================+===============================+================================+===============================================================+
 */
class UpsertProductWithPermissionsWithUuidEndToEnd extends AbstractProductTestCase
{
    /**
     * {@inheritdoc}
     */
...
SQL;
    ...
    {
        $productViewableByEverybody1 = $this->getProductUuidFromIdentifier('product_viewable_by_everybody_1')->toString();
        $productViewableByEverybody2 = $this->getProductUuidFromIdentifier('product_viewable_by_everybody_2')->toString();
        $data = <<<JSON
{
    "associations": {
        "X_SELL": {
            "products": ["{$productViewableByEverybody1}", "{$productViewableByEverybody2}"]
        }
    }
}
JSON;
...
JSON;
    ...
    public function testToLoseTheOwnershipOfAProduct()
    {
        $data = '{"categories": ["categoryA"]}';

        $client = $this->createAuthenticatedClient([], [], null, null, 'mary', 'mary');
        $uuid = $this->getProductUuidFromIdentifier('product_editable_by_redactor')->toString();
