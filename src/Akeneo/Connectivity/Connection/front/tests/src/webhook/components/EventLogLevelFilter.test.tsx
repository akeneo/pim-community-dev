...
test('It display "none" when there is no level selected', () => {
    renderWithProviders(<EventLogLevelFilter levels={[]} onChange={jest.fn()} />);
...
test('It display "all" when all levels are selected', () => {
    ...
    renderWithProviders(<EventLogLevelFilter levels={levels} onChange={jest.fn()} />);
...
test('It display a list when not all levels are selected', () => {
    const levels: EventSubscriptionLogLevel[] = [EventSubscriptionLogLevel.INFO, EventSubscriptionLogLevel.NOTICE];
...
test('It can change the selected levels by opening the dropdown and clicking on one of the checkbox', () => {
    ...
    renderWithProviders(<EventLogLevelFilter levels={[]} onChange={handleChange} />);
    ...
    expect(handleChange).toHaveBeenCalledWith([EventSubscriptionLogLevel.INFO]);
});
