...
const useSuppliers = (search: string, page: number): [SupplierRow[], number, () => void] => {
    ...
    const getSuppliersRoute = useRoute('supplier_portal_retailer_supplier_list');
    const loadSuppliers = useCallback(async () => {
        ...
        if (!response.ok) {
            notify(
                ...
                translate('supplier_portal.supplier.supplier_list.notification.error.title'),
                translate('supplier_portal.supplier.supplier_list.notification.error.content')
            );
