...
import CategoryPermissionFormProvider from './CategoryPermissionFormProvider';
...
test('it renders the form without error', () => {
  render(
    <ThemeProvider theme={pimTheme}>
      <PermissionsForm
        provider={CategoryPermissionFormProvider}
...
test('it saves the permissions without error', async () => {
  await CategoryPermissionFormProvider.save('my_user_group', {
    own: {
      all: false,
      identifiers: [],
    },
  ...
  expect(fetch).toHaveBeenCalledWith('pimee_permissions_entities_set_categories', expect.anything());
});

test('it loads the permissions without error', async () => {
  fetchMock.mockResponseOnce(request => Promise.resolve({body: JSON.stringify({})}));

  await CategoryPermissionFormProvider.loadPermissions('my_user_group');

  expect(fetch).toHaveBeenCalledWith('pimee_permissions_entities_get_user_group_categories', expect.anything());
});
