...
const AddColumnModal: React.FC<AddColumnModalProps> = ({
  ...
  const validateCode = (code: string, silent: boolean): number => {
    ...
    if (code !== '' && !code.match(/^[a-zA-Z0-9_]+$/)) validations.push(translate('pim_table_attribute.validations.invalid_code'));
    if (existingColumnCodes.includes(code)) validations.push(translate('pim_table_attribute.validations.duplicated_column_code', { duplicateCode: code }));
  ...
  const validateDataType = (dataType: ColumnType | null, silent: boolean): number => {
    const validations: string[] = [];
    if (dataType === null) validations.push(translate('pim_table_attribute.validations.column_data_type_must_be_filled'));
    if (!silent) {
      setValidations(oldValidations => {
  ...
  return <Modal closeTitle={translate('pim_common.close')} onClose={close} illustration={<AttributesIllustration />}>
    <Modal.SectionTitle color="brand">{translate('pim_table_attribute.form.attribute.table_attribute')}</Modal.SectionTitle>
    ...
    <FieldsList>
      ...
      <Field label={translate('pim_table_attribute.form.attribute.data_type')} requiredLabel={translate('pim_common.required_label')}>
        <SelectInput
        ...
        >
          {dataTypes.map(dataType => <SelectInput.Option
            ...
            value={dataType}
          >{translate(`pim_table_attribute.properties.data_type.${dataType}`)}</SelectInput.Option>)}
        </SelectInput>
      </Field>
    </FieldsList>
    <Modal.BottomButtons>
