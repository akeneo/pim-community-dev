...
class CompareDiffEsToMySQLCommand extends Command
{
    ...
    {
        $output->writeln("Process to check index begin");
        ...
        foreach ($definedIndexSource as $indexSource) {
            $index = $indexSource->mySql->getTableName();

            $results = $this->readMySQLData($indexSource->mySql);
            $filenameSourceDB = $this->dumpItemToJsonFiles($results, $folder);

            $results = $this->readEsData($indexSource->elasticsearch, $index);
            $filenameSourceES = $this->dumpItemToJsonFiles($results, $folder);

            $status = $this->compareJsonFiles($filenameSourceDB, $filenameSourceES, $index);
    ...
    public function compareJsonFiles(string $mysqlFiles, string $esFiles, string $indexName): int
    {
        ...
        //$this->versionContext->addContextInfo('missing_lines: '.count($resultsDiff->missingLines), $indexName);
        $this->versionContext->addContextInfo($resultsDiff,$indexName);
        $this->logger->notice("index treated", [
