...
resource "google_compute_security_policy" "web_policy" {
  project     = var.project_id
  name        = "${var.project_id}-throttle-web"
  description = "rate limits"
  provider    = google-beta
  dynamic "rule" {
    for_each = toset(var.enable_rate_limiting_web ? [1] : [])
    content {
      action   = "throttle"
      preview  = true
      priority = "1000"
      rate_limit_options {
        ...
        enforce_on_key = "ALL"
        rate_limit_threshold {
          count        = var.rate_limit_web.count
          interval_sec = var.rate_limit_web.interval_sec
        }
      }
      match {
        versioned_expr = "SRC_IPS_V1"
