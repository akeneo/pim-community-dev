...
final class Version_7_0_20221214103600_add_disable_catalogs_on_locale_deactivation_job_instance_Integration extends TestCase
{
    ...
    public function test_it_adds_the_job_instance(): void
    {
        $this->connection->executeStatement(
            <<<SQL
            DELETE FROM akeneo_batch_job_instance WHERE code = 'disable_catalogs_on_locale_deactivation';
            SQL
        );
    ...
    private function jobInstanceExists(): bool
    {
        return (bool) $this->connection->fetchOne(
            <<<SQL
            SELECT EXISTS(
                SELECT * FROM akeneo_batch_job_instance WHERE code = 'disable_catalogs_on_locale_deactivation'
            );
            SQL
        );
    }
