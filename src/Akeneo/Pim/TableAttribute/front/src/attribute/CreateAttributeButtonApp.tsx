...
import {
  ...
  CreateAttributeModalExtraField,
...
const CreateAttributeButtonApp: React.FC<CreateAttributeButtonAppProps> = ({buttonTitle, iconsMap, isModalOpen = false, onClick, defaultCode}) => {
  ...
  if (template && template.templates.length !== 1) {
    extraFields = [{
      component: <Field label={'choose template TODO'} requiredLabel={translate('pim_common.required_label')}>
        <SelectInput
          clearLabel=""
        ...
        >
          {template.templates.map(specific_template => <SelectInput.Option key={specific_template.code} title={specific_template.code} value={specific_template.code}>{specific_template.code}</SelectInput.Option>)}
        </SelectInput>
      </Field>,
  ...
  return (
    <>
      {isSelectAttributeTypeModelOpen && <SelectAttributeTypeModal onClose={handleClose} iconsMap={iconsMap} onAttributeTypeSelect={handleAttributeTypeSelect}/>}
      {isSelectTemplateModalOpen && <SelectTemplateApp onClick={handleTemplateSelect} onClose={handleClose} templates={TEMPLATES}/>}
      {isCreateAttributeModalOpen && <CreateAttributeModal onClose={handleClose} onConfirm={handleConfirm} defaultCode={defaultCode} extraFields={extraFields}/>}
