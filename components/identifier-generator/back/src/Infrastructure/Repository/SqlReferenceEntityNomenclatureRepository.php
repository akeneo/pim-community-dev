...
INNER JOIN akeneo_reference_entity_record r ON r.idenditier = n.record_identifier
...
WHERE reference_entity_identifier = (SELECT id FROM pim_catalog_attribute WHERE code=:attribute_code);
SQL;
...
SQL;
        ...
        foreach (($nomenclatureDefinition->values()) as $recordCode => $value) {
            ...
            if ($recordIdentifier) {
                if (null === $value || '' === $value) {
                    ...
                    $valuesToUpdateOrInsert[] = [
                        'attributeOptionId' => \intval($recordIdentifier),
                        'value' => $value,
                    ];
                }
            }
    ...
    /**
     * @param string[] $recordIdentifiersToDelete
     */
