...
import {TextColumnDefinition} from "../../../../src/models";
import {fireEvent} from "@testing-library/dom";

const selectedColumn = getComplexTableAttribute().table_configuration.find(columnDefinition => columnDefinition.data_type === 'text') as TextColumnDefinition;
...
describe('TextProperties', () => {
  ...
  it('should callback changes', () => {
    ...
    renderWithProviders(
      <TextProperties attribute={getComplexTableAttribute()} selectedColumn={selectedColumn} handleChange={handleChange}/>
    );
    ...
    expect(handleChange).toBeCalledWith({...selectedColumn, validations: {
      max_length: 10,
    }});
  });
  ...
  it('should display error message when max length is too big', () => {
    ...
    renderWithProviders(
      <TextProperties attribute={getComplexTableAttribute()} selectedColumn={erroredSelectedColumn} handleChange={jest.fn()}/>
    );
  ...
  it('should unset validations when values are not filled anymore', () => {
    ...
    const handleChange = jest.fn();
    renderWithProviders(
      <TextProperties attribute={getComplexTableAttribute()} selectedColumn={filledSelectedColumn} handleChange={handleChange}/>
    );
