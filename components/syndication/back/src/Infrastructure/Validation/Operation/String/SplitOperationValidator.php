...
class SplitOperationValidator extends ConstraintValidator
{
    public function validate($operation, Constraint $constraint): void
    {
        $validator = $this->context->getValidator();

        $violations = $validator->validate($operation, new Optional(new Collection([
            'fields' => [
                'type' => new EqualTo(['value' => 'split']),
                'seperator' => [
                    new Type('string'),
                    new Length([
                        'max' => 255,
