  ...
  return BaseForm.extend({
    ...
    initializeSelect: function() {
        ...
        query: function(options) {
          ...
          this.queryTimer = setTimeout(
            function() {
              ...
              FetcherRegistry.getFetcher('contributor')
                .search(searchParameters)
                .then(
                  function(contributors) {
                    var choices = this.arrayToSelect2Format(contributors).map(choice =>
                      this.updateCurrentUsername(choice)
                    );
      ...
      $select.on('select2-open', function() {
        $('.teamwork-assistant-widget-contributor-dropdown .select2-input').attr(
          'placeholder',
          __('teamwork_assistant.widget.placeholder.contributor_selector')
        );
      });
