...
/**
 * Filter product model's association to have only updated or new values
 *
 * @author    Adrien Pétremann <adrien.petremann@akeneo.com>
 * @copyright 2018 Akeneo SAS (https://www.akeneo.com)
 * @license   http://opensource.org/licenses/osl-3.0.php  Open Software License (OSL 3.0)
 */
class ProductModelAssociationFilter implements FilterInterface
{
     ...
     * {@inheritdoc}
     */
    public function filter(EntityWithValuesInterface $productModel, array $newValues): array
    {
        $originalAssociations = $this->normalizer->normalize($productModel, 'standard');
        ...
        $result = [];
        foreach ($newValues[ProductModelNormalizer::FIELD_ASSOCIATIONS] as $type => $field) {
            foreach ($field as $key => $association) {
                $data = $this->compareAssociation($originalAssociations, $association, $type, $key);

                if (null !== $data) {
                    $result[ProductModelNormalizer::FIELD_ASSOCIATIONS][$type][$key] = $data;
                }
            }
        }
    ...
    {
        $comparator = $this->comparatorRegistry->getFieldComparator(ProductModelNormalizer::FIELD_ASSOCIATIONS);
     ...
     * @param array  $originalAssociations original associations
     * @param string $type                 type of association (PACK, SUBSTITUTION, etc)
     * @param string $key                  key of group (products, groups or product_models)
     *
     * @return array
     */
