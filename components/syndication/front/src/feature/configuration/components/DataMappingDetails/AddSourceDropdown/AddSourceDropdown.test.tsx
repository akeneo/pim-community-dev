...
test('it adds attribute source', async () => {
  ...
  renderWithProviders(<AddSourceDropdown canAddSource={true} onSourceSelected={handleSourceSelected} type="string" />);
  await act(async () => {
    userEvent.click(screen.getByText('akeneo.syndication.data_mapping_details.sources.add'));
  });
...
test('it cannot add a source when the limit is reached', () => {
  ...
  renderWithProviders(<AddSourceDropdown canAddSource={false} onSourceSelected={handleSourceSelected} type="string" />);
