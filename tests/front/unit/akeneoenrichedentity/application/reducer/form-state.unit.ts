import reducer from 'akeneoenrichedentity/application/reducer/form-state';
...
describe('akeneo > enriched entity > application > reducer --- edit-form', () => {
  ...
  test('I can receive original data', () => {
    ...
    expect(newState).toEqual({"entity": {"identifier": "michel", "label": "Michel"}, 'isDirty': false, 'originalData': JSON.stringify(entity)});
  });
  ...
  test('I can invalid the edit form', () => {
    ...
    expect(newState).toEqual({"entity": {"identifier": "michel", "label": "Michel"}, 'isDirty': true, 'originalData': JSON.stringify(entity)});
  });
});
