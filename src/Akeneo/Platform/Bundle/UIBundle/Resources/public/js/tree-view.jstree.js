define([
  ...
  var getTreeUrl = function () {
    return Routing.generate(getRoute('listtree'), {
      _format: 'json',
  ...
  var getChildrenUrl = function () {
    return Routing.generate(getRoute('children'), {
      _format: 'json',
      dataLocale: dataLocale,
      context: 'view',
    });
  ...
  var getTreeConfig = function () {
    return {
      ...
      plugins: ['tree_selector', 'nested_switch', 'themes', 'json_data', 'ui', 'crrm', 'types'],
      nested_switch: {
        state: includeSub,
