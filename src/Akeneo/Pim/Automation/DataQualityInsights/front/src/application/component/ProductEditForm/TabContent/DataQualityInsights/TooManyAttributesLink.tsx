  ...
  if (isSimpleProduct(product) || isVariantProduct(product) || isRootProductModel(product)) {
    switch (axis) {
      case 'enrichment':
        ...
        break;
      case 'consistency':
        window.dispatchEvent(new CustomEvent(DATA_QUALITY_INSIGHTS_FILTER_ALL_IMPROVABLE_ATTRIBUTES, {
          detail: {
            attributes: attributes,
          }
        }));
    ...
    if(! product.meta.hasOwnProperty('parent_attributes') || ! product.meta.parent_attributes.includes(attributeToImprove)) {
