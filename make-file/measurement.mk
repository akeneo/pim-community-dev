...
measurement-lint-back: #Doc: launch PHPStan for Measurement bounded context
	${PHP_RUN} vendor/bin/rector process --dry-run --config src/Akeneo/Tool/Bundle/MeasureBundle/tests/rector.php

.PHONY: measurement-lint-fix-back
measurement-lint-fix-back: #Doc: launch PHPStan for Measurement bounded context
	${PHP_RUN} vendor/bin/rector process --config src/Akeneo/Tool/Bundle/MeasureBundle/tests/rector.php
...
ifeq ($(CI),true)
	tests/scripts/run_phpunit.sh . tests/scripts/find_phpunit.php Akeneo_Measurement_Acceptance
else
