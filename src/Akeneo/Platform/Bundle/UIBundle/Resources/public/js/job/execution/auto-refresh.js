...
define([
  ...
  return BaseForm.extend({
    ...
    restartAutoRefreshTimeout: function () {
      ...
      this.autoRefreshTimeout = setTimeout(this.fetchData.bind(this, this.getFormData()), this.autoRefreshDelay);
    },
    ...
    startAutoUpdate: function () {
      ...
      this.stopAutoUpdateTimeout = setTimeout(
        function () {
          this.stopAll();
          this.setStatus('isNotFinished');
        }.bind(this),
        this.stopAutoRefreshDelay
      );
    },
    ...
    fetchData: function (jobExecution) {
      if ('Stopping' === jobExecution.status || jobExecution.isRunning) {
        ...
        FetcherRegistry.getFetcher('job-execution')
          .fetch(jobId, {id: jobId, cached: false})
          .then(
