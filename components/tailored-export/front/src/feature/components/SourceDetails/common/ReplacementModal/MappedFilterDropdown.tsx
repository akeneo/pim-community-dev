  ...
  return (
    <Dropdown>
      <SwitcherButton
        ...
        onClick={open}
      >
        {translate(`akeneo.tailored_export.column_details.sources.operation.replacement.modal.filters.mapped.${value}`)}
      </SwitcherButton>
      {isDropdownOpen && (
        <Dropdown.Overlay verticalPosition="down" onClose={close}>
          <Dropdown.ItemCollection>
            {mappedFilterValues.map(filterValue => (
              ...
              >
                {translate(
                  `akeneo.tailored_export.column_details.sources.operation.replacement.modal.filters.mapped.${filterValue}`
                )}
              </Dropdown.Item>
            ))}
          </Dropdown.ItemCollection>
