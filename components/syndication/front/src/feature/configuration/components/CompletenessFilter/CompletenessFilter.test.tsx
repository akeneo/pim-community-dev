...
jest.mock('../../hooks/pim/useChannels', () => ({
...
test.each(operatorsAndVisibility)(
  ...
  ({operator, shouldAppear}: {operator: Operator; shouldAppear: boolean}) => {
    ...
    if (shouldAppear) {
      ...
      expect(screen.getByText('akeneo.syndication.filters.completeness.locales.label')).toBeInTheDocument();
    } else {
      // eslint-disable-next-line jest/no-conditional-expect
      expect(screen.queryByText('akeneo.syndication.filters.completeness.locales.label')).not.toBeInTheDocument();
    }
  }
);
...
test('it can select locales', () => {
  ...
  userEvent.click(screen.getByLabelText('akeneo.syndication.filters.completeness.locales.label'));
