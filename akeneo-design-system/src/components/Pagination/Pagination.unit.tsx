...
test('it renders a maximum of 7 pagination item when there is 7 or less pages', () => {
  const result = render(<Pagination currentPage={1} itemsTotal={14} itemsPerPage={2} onClick={jest.fn()} />);

  expect(screen.getAllByTestId('paginationItem')).toHaveLength(7);
  expect(result.container.textContent).toBe(expectedPagination(['1', '2', '3', '4', '5', '6', '7']));
});
...
test('it renders the 4 first pagination item and the last page if the current page is the third one', () => {
  ...
  expect(screen.getAllByTestId('paginationItem')).toHaveLength(6);
...
test('it ensures separator click does nothing', () => {
  ...
  render(<Pagination currentPage={12} itemsTotal={50} itemsPerPage={2} onClick={onClickSeparator} />);

  //Expected pagination ['1', '...', '11', '12', '13', '...', '25']
