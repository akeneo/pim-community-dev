const AttributeEdit = async (nodeElement, createElementDecorator, page) => {
	...
	const disabledLabel = async () => {
		try {
			await page.waitForSelector(
				'.AknFieldContainer[data-code="label"] .AknTextField--disabled'
			);
		} catch (error) {
			return false;
		}
	...
	const disabledIsRequired = async () => {
		try {
			await page.waitForSelector(
				'.AknFieldContainer[data-code="isRequired"] .AknCheckbox--disabled'
			);
		} catch (error) {
			return false;
		}
	...
	const disabledIsTextarea = async () => {
		try {
			await page.waitForSelector(
				'.AknFieldContainer[data-code="isTextarea"] .AknCheckbox--disabled'
			);
		} catch (error) {
			return false;
		}
	...
	const disabledValidationRule = async () => {
		try {
			await page.waitForSelector(
				'.AknFieldContainer[data-code="validationRule"] .AknDropdown-menu',
				{ hidden: true }
			);
		} catch (error) {
			return false;
	...
	const setRegularExpression = async value => {
		const regularExpression = await nodeElement.$(
	...
	const setMaxLength = async value => {
		const maxLength = await nodeElement.$(
	...
	const disabledMaxLength = async () => {
		try {
			await page.waitForSelector(
				'.AknFieldContainer[data-code="maxLength"] .AknTextField--disabled'
			);
		} catch (error) {
			return false;
		}
	...
	const setMaxFileSize = async value => {
		const maxFileSize = await nodeElement.$(
	...
	const disabledMaxFileSize = async () => {
		try {
			await page.waitForSelector(
				'.AknFieldContainer[data-code="maxFileSize"] .AknTextField--disabled'
			);
		} catch (error) {
			return false;
		}
	...
	const setAllowedExtensions = async value => {
		const allowedExtensions = await nodeElement.$(
			'.AknFieldContainer[data-code="allowedExtensions"] select'
		);
		await page.evaluate(
				...
				Array.prototype.slice
					.call(
						properties.querySelectorAll(
							'.AknFieldContainer[data-code="allowedExtensions"] option'
						)
					)
					.forEach(
						option => (option.selected = newValues.includes(option.value))
					);
			},
			nodeElement,
			value.split(",")
	...
	const disabledAllowedExtensions = async () => {
		try {
			await page.waitForSelector(
				'.AknFieldContainer[data-code="allowedExtensions"] select:disabled'
			);
		} catch (error) {
			return false;
		}
	...
	const setMinValue = async value => {
		const minValue = await nodeElement.$(
	...
	const setMaxValue = async value => {
		const maxValue = await nodeElement.$(
	...
	const isVisible = async property => {
		return (
			null !==
			(await nodeElement.$(`.AknFieldContainer[data-code="${property}"]`))
		);
	};
