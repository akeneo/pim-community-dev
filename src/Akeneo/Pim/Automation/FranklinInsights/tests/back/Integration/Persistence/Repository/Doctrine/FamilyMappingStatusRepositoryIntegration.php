 ...
 */
final class FamilyMappingStatusRepositoryIntegration extends TestCase
{
    ...
    public function test_that_families_with_subscribed_products_are_found_and_paginated(): void
    {
        ...
        $this->assertFamilyCollection(
            $familyCollection,
            [
                $this->buildFamilyMappingStatus(self::TEST_FAMILY_CODE, self::TEST_FAMILY_LABELS, FamilyMappingStatus::MAPPING_PENDING),
                $this->buildFamilyMappingStatus(self::CONTROL_FAMILY_CODE, self::CONTROL_FAMILY_LABELS, FamilyMappingStatus::MAPPING_FULL),
            ]
        );
    ...
    public function test_that_families_without_translation_are_found(): void
    {
        ...
        $this->assertFamilyCollection(
            ...
            [
                $this->buildFamilyMappingStatus(self::CONTROL_FAMILY_CODE, self::CONTROL_FAMILY_LABELS, FamilyMappingStatus::MAPPING_FULL),
                $this->buildFamilyMappingStatus(self::ONLY_CODE_FAMILY_CODE, [0 => null], FamilyMappingStatus::MAPPING_PENDING),
            ]
        );
    }
    ...
    public function test_that_only_families_with_subscribed_products_are_fetched(): void
    {
        ...
        $this->assertFamilyCollection(
            $familyCollection,
            [$this->buildFamilyMappingStatus(self::TEST_FAMILY_CODE, self::TEST_FAMILY_LABELS, FamilyMappingStatus::MAPPING_FULL)]
        );
...
SQL;
    ...
    /**
     * @return FamilyMappingStatusRepository
     */
    private function getRepository(): FamilyMappingStatusRepository
    {
        return $this->get('akeneo.pim.automation.franklin_insights.repository.family_mapping_status');
    }

    /**
     * @param FamilyMappingStatusCollection $familyCollection
     * @param FamilyMappingStatus[]         $expectedFamilies
     */
    private function assertFamilyCollection(FamilyMappingStatusCollection $familyCollection, array $expectedFamilies): void
    {
        Assert::assertCount(count($expectedFamilies), $familyCollection);
        foreach ($familyCollection as $position => $familyMappingStatus) {
            Assert::assertEquals($expectedFamilies[$position], $familyMappingStatus);
        }
    }

    /**
     * @param string $familyCode
     * @param array  $familyLabels
     * @param int    $mappingStatus
     *
     * @return FamilyMappingStatus
     */
    private function buildFamilyMappingStatus(string $familyCode, array $familyLabels, int $mappingStatus): FamilyMappingStatus
    {
        return new FamilyMappingStatus(
            new Family(new FamilyCode($familyCode), $familyLabels),
            $mappingStatus
        );
    }
}
