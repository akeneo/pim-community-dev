...
test('it translates sentence with Component', () => {
  ...
  expect(current).toEqual(<>{['To know more about akeneo, ', <a key="link">this article may help you</a>]}</>);
});

test('it translates sentence with Component and placeholder', () => {
  const {result} = renderHookWithProviders(() => useTranslateWithComponents());
  const translate = result.current;
  const current: ReactNode = translate(
  ...
  expect(current).toEqual(
    <>{['To know more about ', <b key="b">akeneo</b>, ', ', <a key="link">read more</a>, ' !']}</>
  );
});
...
test('it translates sentence with Component and placeholder', () => {
  ...
  expect(current).toEqual(<>{['To know more about akeneo, <unknown>this article may help you</unknown>']}</>);
});
