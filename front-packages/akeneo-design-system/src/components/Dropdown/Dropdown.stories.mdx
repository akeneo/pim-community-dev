...
import {SpaceContainer, useSelection, fakeFetcher} from '../../storybook';
...
For search and pagination, you need to create a dedicated component for the `Dropdown.Overlay` to avoid re-fetching data even if the overlay is closed.
  ...
  <Story name="Search & pagination" height={500}>
    {args => {
      ...
      const Overlay = () => {
        ...
        const [items, handleNextPage] = usePaginatedResults(page => fakeFetcher(page, debouncedSearchValue), [
