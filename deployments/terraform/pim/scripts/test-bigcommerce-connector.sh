...
${CURL} -L -m 5 -w "%{http_code}" ${TARGET}/connectors/bigcommerce/api-web/service-status | grep 403

echo "Testing front for bigcommerce connector"
${CURL} -L -m 5 -w "%{http_code}" ${TARGET}/connectors/bigcommerce/configuration | grep 200
