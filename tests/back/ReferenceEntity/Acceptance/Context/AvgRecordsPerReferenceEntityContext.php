...
final class AvgRecordsPerReferenceEntityContext implements Context
{
    /** @var ReportContext */
    ...
    /**
     * @Given a catalog with on average :recordsPerReferenceEntity records per reference entity
     */
    ...
    /**
     * @Given the average limit of records per reference entity is set to :limit
     */
    ...
    /**
     * @Then the report returns that the average number of records per reference entityis :recordsPerReferenceEntity
     */
    public function theReportReturnsThatTheRecordsPerReferenceEntityIs(int $recordsPerReferenceEntity): void
    {
        $volumes = $this->reportContext->getVolumes();

        Assert::eq($recordsPerReferenceEntity, $volumes['average_records_per_reference_entities']['value']);
    }
    ...
    public function theReportWarnsTheUsersThatTheRecordsPerReferenceEntityIsHigh(): void
    {
        ...
        Assert::true($volumes['average_records_per_reference_entities']['has_warning']);
    }

    /**
     * @Then the report does not warn the users that the average number of records per reference entity is high
     */
    public function theReportDoesNotWarnTheUsersThatTherecordsPerReferenceEntityIsHigh(): void
    {
        $volumes = $this->reportContext->getVolumes();

        Assert::false($volumes['average_records_per_reference_entities']['has_warning']);
    }
}
