...
class IdFilterIntegration extends AbstractProductQueryBuilderTestCase
{
    ...
    protected function setUp()
    {
        ...
        if (1 === self::$count || $this->getConfiguration()->isDatabasePurgedForEachTest()) {
            ...
            self::$ids['foo'] = $foo->getId();
            self::$ids['bar'] = $bar->getId();
            self::$ids['baz'] = $baz->getId();
            self::$ids['barista'] = $barista->getId();
            self::$ids['bazar'] = $bazar->getId();
        }
    }
     ...
     * @expectedExceptionMessage Property "id" expects a numeric as data, "array" given.
     */
    ...
    /**
     * @expectedException \Akeneo\Component\StorageUtils\Exception\InvalidPropertyTypeException
     * @expectedExceptionMessage Property "id" expects an array with valid data, one of the value is not numeric.
     */
    public function testDataIsNotAListOfNumericForOperatorInList()
    {
     ...
     * @expectedExceptionMessage Property "id" expects an array with valid data, one of the value is not numeric.
     */
    public function testDataIsNotAListOfNumericForOperatorNotInList()
    {
