...
namespace Specification\Akeneo\Platform\TailoredImport\Infrastructure\FlatFileIterator;
...
class CellsFormatterSpec extends ObjectBehavior
{
    ...
    {
        $this->format(['foo', 'bar'])->shouldReturn(['foo', 'bar']);
    }

    public function it_formats_number_cells()
    {
        $this->format([12, 25.5])->shouldReturn(['12', '25.5']);
    }

    public function it_formats_boolean_cells()
    {
        $this->format([true, false])->shouldReturn(['TRUE', 'FALSE']);
    }
    ...
    {
        $this->shouldThrow(\RuntimeException::class)->during('format', [[new \stdClass()]]);
    }

    public function it_throws_an_exception_when_cell_is_invalid()
    {
        $this->shouldThrow(\RuntimeException::class)->during('format', [[null]]);
    }
}
