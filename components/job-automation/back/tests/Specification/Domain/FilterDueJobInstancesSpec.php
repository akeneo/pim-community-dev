...
class FilterDueJobInstancesSpec extends ObjectBehavior
{
    ...
    public function it_returns_only_due_scheduled_jobs_based_on_its_cron_expression(
        ...
        $scheduledJobInstances = [
            ...
            new ScheduledJobInstance('job_1', 'a_job','import', [], [], [], '0 0/4 * * *', new \DateTimeImmutable('2022-10-30 00:00'), null, 'job_automated_job_1'),
            // Last execution date is in the past, so should be due
            new ScheduledJobInstance('job_2', 'a_job','import', [], [], [], '0 0/12 * * *', new \DateTimeImmutable('2022-10-30'), new \DateTimeImmutable('2022-10-30 00:00'), 'job_automated_job_2'),
            // Too early for the next execution, so should not be due (next will be at 16:00)
            new ScheduledJobInstance('job_3', 'a_job','import', [], [], [], '0 0/8 * * *', new \DateTimeImmutable('2022-10-30'), new \DateTimeImmutable('2022-10-30 08:00'), 'job_automated_job_3'),
            // Configured too late to be run, so should not be due (next will be at 14:00)
            new ScheduledJobInstance('job_4', 'a_job','import', [], [], [], '0 0/2 * * *', new \DateTimeImmutable('2022-10-30 12:34'), null, 'job_automated_job_4'),
        ];
