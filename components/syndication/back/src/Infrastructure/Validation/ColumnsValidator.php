...
use Akeneo\Platform\Syndication\Infrastructure\Validation\Target\Target;
    ...
    public function validate($columns, Constraint $constraint): void
    {
        ...
        $violations = $validator->validate($columns, [
            ...
            new Count([
                ...
                'maxMessage' => 'akeneo.syndication.validation.columns.max_column_count_reached'
            ])
        ]);
        ...
        foreach ($columns as $column) {
            ...
            if (isset($column['target']['name'])) {
                if (in_array($column['target']['name'], $columnTargets)) {
                    $this->context->buildViolation(Target::TARGET_NAME_SHOULD_BE_UNIQUE)
                        ->atPath(sprintf('[%s][target]', $column['uuid']))
                        ->setInvalidValue($column['target'])
                        ->addViolation();
                } else {
                    $columnTargets[] = $column['target']['name'];
                }
            }
        }
    }
