...
describe('>>>REDUCER --- user', () => {
  ...
  test('handle switch on new locale', () => {
    expect(reducer(undefined, {type: 'LOCALE_CHANGED', target: 'copy', locale: 'fr_FR'})).toEqual({
      copyLocale: 'fr_FR',
    });
  });

  test('handle switch on existing target locale', () => {
    expect(reducer({catalogLocale: 'fr_FR'}, {type: 'LOCALE_CHANGED', target: 'catalog', locale: 'en_US'})).toEqual({
      catalogLocale: 'en_US',
    });
  });
  ...
  test('handle switch on existing target channel', () => {
    expect(
      reducer(
        ...
        {type: 'CHANNEL_CHANGED', target: 'catalog', channel: 'ecommerce'}
      )
    ).toEqual({catalogLocale: 'fr_FR', catalogChannel: 'ecommerce'});
  });

  test('handle switch on existing wrong value', () => {
    expect(reducer({}, {type: 'CHANNEL_CHANGED', target: 'catalog', locale: 'ecommerce'})).toEqual({});
  });
});
