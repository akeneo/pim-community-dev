...
{
    const FORMATTER_EXTENSION_ID = 'oro_grid.extension.formatter';
    const TAG_NAME               = 'oro_grid.extension.formatter.property';
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        if ($extension) {
            ...
            foreach ($properties as $serviceId => $tags) {
                $tagAttrs = reset($tags);
                $extension->addMethodCall('addProperty', array($tagAttrs['type'], new Reference($serviceId)));
            }
        }
    }
}
