...
class V20220729171405DropProductIdColumnsAndCleanVersioningResourceUuidColumns implements ZddMigration
{
    ...
    private function dropTriggersAndProductIdColumns(string $table, array $tableProperties): void
    {
        ...
        if (null !== $tableProperties['column'] && $this->columnExists($table, $tableProperties['column'])) {
            $this->dropForeignKeys($table, $tableProperties['column']);
            $this->dropIndexes($table, $tableProperties['column']);
            $this->dropColumn($table, $tableProperties['column']);
        }

        $triggersDropped = false;
        $retries = 0;
        while (false === $triggersDropped) {
            try {
                $this->dropTriggers($tableProperties['triggers']);
                $triggersDropped = true;
            } catch (\Exception $e) {
                // we don't want the triggers to be there if the column was dropped, so we retry a few times
                \sleep(2); // in case the connection was lost and needs a few seconds to be restored

                if ($retries < 10) {
                    $retries++;
                    $this->logger->error(
                        \sprintf(
                            'An error occurred while trying to drop the triggers: "%s", retrying in 2 seconds',
                            $e->getMessage()
                        ),
                        ['zdd_migration' => $this->getName()]
                    );
                } else {
                    throw $e;
                }
