...
const useInitialDataGridState = <T extends {}>(
    ...
    const move = useCallback((source: T, target: T, direction: MoveDirection) => {
    ...
    const moveUp = useCallback((source: T, target: T) => {
        move(source, target, MoveDirection.Up);
    }, [move]);

    const moveDown = useCallback((source: T, target: T) => {
        move(source, target, MoveDirection.Down);
    }, [move]);

    const isDragged = useCallback((data: T) => {
        if (draggedData === null) {
            return false;
        }
    ...
    const moveStart = useCallback((event: React.DragEvent, source: T, index: number, rowElement: Element|null) => {
    ...
    const moveEnd = useCallback((event: React.DragEvent) => {
        handleDragEnd(event, () => {
            setDraggedData(null);
            setDraggedIndex(DEFAULT_DRAGGED_INDEX);
        });
