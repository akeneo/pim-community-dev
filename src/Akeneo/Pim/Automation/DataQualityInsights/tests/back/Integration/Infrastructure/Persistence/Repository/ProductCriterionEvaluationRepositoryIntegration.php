...
final class ProductCriterionEvaluationRepositoryIntegration extends DataQualityInsightsTestCase
{
    ...
    public function test_it_creates_a_collection_of_product_criteria_evaluations()
    {
        ...
        $productUuid = new ProductUuid($product->getUuid());
    ...
    {
        $productUuidWithExistingEvaluation = new ProductUuid($this->createProduct('product_with_evaluation')->getUuid());
        $productUuidWithoutEvaluation = new ProductUuid($this->createProduct('product_without_evaluation')->getUuid());
    ...
    public function test_it_updates_product_criteria_evaluations()
    {
        ...
        $productUuid = new ProductUuid($this->createProduct('ziggy')->getUuid());
    ...
    private function buildCollection(): Write\CriterionEvaluationCollection
    {
        return (new Write\CriterionEvaluationCollection)
            ->add(new Write\CriterionEvaluation(
                new CriterionCode('completeness'),
                new ProductUuid($this->createProduct('a_product')->getUuid()),
                CriterionEvaluationStatus::pending()
            ))
            ->add(new Write\CriterionEvaluation(
                new CriterionCode('completion'),
                new ProductUuid($this->createProduct('another_product')->getUuid()),
                CriterionEvaluationStatus::pending()
            ));
    }
