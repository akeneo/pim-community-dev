...
class SqlVariantProductsIntegration extends BuilderQueryTestCase
{
    public function testGetCountOfProducts()
    {
        $query = $this->get('pim_volume_monitoring.persistence.query.variant_products');
        $this->createVariantProducts(8);

        $volume = $query->fetch();

        Assert::assertEquals(8, $volume->getVolume());
        Assert::assertEquals('variant_products', $volume->getVolumeName());
