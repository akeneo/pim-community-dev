...
class Version20151005132540 extends AbstractMigration
{
    ...
    public function down(Schema $schema)
    {
        ...
        $this->addSql('ALTER TABLE oro_user DROP FOREIGN KEY FK_F82840BCF925933B');
        $this->addSql('DROP INDEX IDX_F82840BCF925933B ON oro_user');
        $this->addSql('ALTER TABLE oro_user DROP ui_locale_id');
    }

    protected function getCatalogLocaleId($code, $like = false)
    {
        $query = "SELECT id FROM " . self::CATALOG_LOCALE_TABLE . " WHERE is_activated=1 AND ";
    ...
    protected function getDefaultCatalogLocaleId()
    {
        ...
        if (null !== $result['value']) {
            ...
            $full_locale = $this->getDefaultFullLocale($code);
            $result = $this->getCatalogLocaleId($full_locale);
            if (null !== $result) {
                return $result;
            }
