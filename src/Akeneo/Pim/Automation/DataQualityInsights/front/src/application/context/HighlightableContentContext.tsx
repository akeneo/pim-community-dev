...
export const HighlightableContentContextProvider: FC<HighlightableContentContextProviderProps> = ({children, element}) => {
 const providerState = useHighlightableContentContextState(element);

  return (
    <HighlightableContentContext.Provider value={providerState}>
      {children}
    </HighlightableContentContext.Provider>
  )
}
