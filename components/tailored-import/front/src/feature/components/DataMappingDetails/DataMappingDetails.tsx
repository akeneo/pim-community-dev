...
const DataMappingDetails = ({fileKey, fileStructure, columns, dataMapping, validationErrors, onDataMappingChange}: DataMappingDetailsProps) => {
  ...
  const handleSourcesChange = async (sources: ColumnIdentifier[]) => {
    ...
    if (sources.length > 0 && null !== column) {
        sample_data = await fetchSampleData(fileKey, column.index, fileStructure.sheet_name, fileStructure.first_product_row);
    } else {
        sample_data = [];
    }
