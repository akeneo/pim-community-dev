/* global define */
define(['underscore', 'backbone', 'backbone/pageable-collection', 'oro/app'], function (
  _,
  Backbone,
  ...
  var PageableCollection = BackbonePageableCollection.extend({
    ...
    processFiltersParams: function (data, state, prefix) {
      ...
      if (this.inputName && !_.isUndefined(data[this.inputName]) && !_.isUndefined(data[this.inputName]._filter)) {
        const scope = !_.isUndefined(data[this.inputName]._filter.scope) ? data[this.inputName]._filter.scope : null;
    ...
    generateParameterStrings: function (parameters, prefix) {
      ...
      _.each(
        parameters,
        function (filterParameters, filterKey) {
    ...
    parse: function (resp, options) {
      this.state.totalRecords =
        'undefined' !== typeof resp.totalRecords ? resp.totalRecords : resp.options.totalRecords;
        ...
        if (mode == 'infinite') {
          ...
          if (firstPage === 0 && (currentPage < firstPage || currentPage >= totalPages)) {
            throw new RangeError(
              '`currentPage` must be firstPage <= currentPage < totalPages if 0-based. Got ' + currentPage + '.'
            );
          } else if (firstPage === 1 && (currentPage < firstPage || currentPage > totalPages)) {
            throw new RangeError(
              '`currentPage` must be firstPage <= currentPage <= totalPages if 1-based. Got ' + currentPage + '.'
            );
          }
      ...
      if (mode != 'server') {
        ...
        options.success = function (col, resp, opts) {
          ...
          if (mode == 'client') resetQuickly(fullCollection, models, opts);
            ...
            fullCollection.update(fullModels, _.extend({silent: true, sort: false}, opts));
            if (fullCollection.comparator) fullCollection.sort();
            fullCollection.trigger('reset', fullCollection, opts);
          } else {
            // fetching new page
            fullCollection.add(models, _.extend({at: fullCollection.length, silent: true}, opts));
            fullCollection.trigger('reset', fullCollection, opts);
          }
    ...
    setSorting: function (sortKey, order, options) {
      ...
      var mode = this.mode;
      options = _.extend({side: mode == 'client' ? mode : 'server', full: true}, options);
