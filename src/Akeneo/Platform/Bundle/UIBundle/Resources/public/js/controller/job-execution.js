...
define(['underscore', 'pim/controller/front', 'pim/form-builder', 'pim/fetcher-registry'], function (
  ...
  return BaseController.extend({
    ...
    renderForm: function (route) {
      return FetcherRegistry.getFetcher('job-execution')
        .fetch(route.params.id, {id: route.params.id, cached: false})
        .then(jobExecution => {
          if (!this.active) {
            return;
          }

          return FormBuilder.build('pim-job-execution-form').then(form => {
