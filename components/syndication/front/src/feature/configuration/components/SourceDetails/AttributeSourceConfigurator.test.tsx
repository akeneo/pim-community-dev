...
import {AttributeSource} from '../../models';
...
test('it displays source configurator', async () => {
  ...
  expect(screen.getByText(/akeneo.syndication.data_mapping_details.sources.operation.header/i)).toBeInTheDocument();
});
...
test('it renders an invalid attribute placeholder when the source is invalid', async () => {
  ...
  expect(
    screen.getByText('akeneo.syndication.data_mapping_details.sources.invalid_source.attribute')
  ).toBeInTheDocument();
  mockedConsole.mockRestore();
});
