 ...
 */
class SaveProductModelTwoWayAssociations implements EventSubscriberInterface
{
    /**
     * @var Connection
     */
        ...
        foreach ($productModel->getAssociations() as $association) {
            if ($association->getAssociationType()->isBidirectional()) {
                $this->connection->transactional(function() use ($productModel, $association) {
                    $this->saveInvertedProductModelAndProductAssociation($association, $productModel);
                    $this->saveInvertedProductModelAndProductModelAssociation($association, $productModel);
                });
            }
        }
    }

    private function saveInvertedProductModelAndProductAssociation(
        AssociationInterface $association,
        ProductModelInterface $productModel
    ): void {
        $this->removeInvertedProductAssociationsDeleted($association, $productModel);
        $this->saveBidirectionalProductAssociation($association, $productModel->getId());
    }

    private function saveInvertedProductModelAndProductModelAssociation(
        AssociationInterface $association,
        ProductModelInterface $productModel
    ) {
        $this->removeInvertedProductModelAssociationsDeleted($association, $productModel);
        $this->saveBidirectionalProductModelAssociation($association, $productModel->getId());
    }
...
SQL;
    ...
    private function saveBidirectionalProductModelAssociation(
        AssociationInterface $productAssociation,
        $ownerProductModelId
    ): void {
    ...
    private function saveNewProductModelAssociation(AssociationInterface $productAssociation, $productModelId)
    {
...
SQL;
    ...
    private function fetchProductModelAssociationId(int $associationId, $productModelId)
    {
    ...
    private function saveProductAssociation($newAssociationId, $ownerProductModelId)
    {
...
SQL;
    ...
    private function saveProductModelAssociation($newAssociationId, $ownerProductModelId)
    {
        $insertProductAssociation = <<<SQL
