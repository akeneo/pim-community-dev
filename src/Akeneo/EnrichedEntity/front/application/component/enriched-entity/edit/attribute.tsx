...
const renderAttributesPlaceholder = () => {
  return Array(8).fill('placeholder').map((attributeIdentifier, key) => (
    <div key={key} className="AknFieldContainer">
      <div className="AknFieldContainer-header">
        ...
        >
          <img
            ...
            src={`bundles/pimui/images/attribute/icon-text.svg`}
          />
          <span>
            {__(`pim_enriched_entity.attribute.type.text`)}{' '}
            {`(${__('pim_enriched_entity.attribute.is_required')})`}
          </span>
        </label>
      </div>
...
interface AttributeViewProps {attribute: NormalizedAttribute; onAttributeEdit: (attribute: AttributeModel) => void; onAttributeDelete: (attribute: AttributeModel) => void; locale: string};

class AttributeView extends React.Component<AttributeViewProps> {
  public shouldComponentUpdate(nextProps: AttributeViewProps) {
    return nextProps.attribute.labels[nextProps.locale] !== this.props.attribute.labels[this.props.locale] ||
          ...
          {this.props.firstLoading || 0 < this.props.attributes.length ? (
            <div className="AknSubsection-container">
              <div className="AknFormContainer AknFormContainer--withPadding">
                {this.props.firstLoading ? renderAttributesPlaceholder() : (
                  <React.Fragment>
                    {this.props.attributes.map((attribute: NormalizedAttribute) => <AttributeView attribute={attribute} onAttributeEdit={this.props.events.onAttributeEdit} onAttributeDelete={this.props.events.onAttributeDelete} locale={this.props.context.locale} />)}
                    <button
