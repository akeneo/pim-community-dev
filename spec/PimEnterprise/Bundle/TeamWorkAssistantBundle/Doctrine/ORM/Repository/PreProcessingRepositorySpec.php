...
class PreProcessingRepositorySpec extends ObjectBehavior
{
    ...
    function it_adds_the_pre_processed_completeness_for_product(
        ...
        $tableNameMapper->getTableName('pimee_team_work_assistant.completeness_per_attribute_group')
            ->willReturn('pimee_team_work_assistant_completeness_per_attribute_group');
    ...
    function it_adds_products_to_a_project(
        ...
        $tableNameMapper->getTableName('pimee_team_work_assistant.project_product')
            ->willReturn('pimee_team_work_assistant_project_product');

        $connection->insert('pimee_team_work_assistant_project_product', [
            'project_id' => 13,
            'product_id' => 37,
        ])->shouldBeCalled();
    ...
    function it_prepares_the_project_calculation_by_deleting_associated_products(
        ...
        $connection->delete('pimee_team_work_assistant_project_product', [
            'project_id' => 40,
        ])->shouldBeCalled();
    ...
    function it_links_between_product_and_category(
        ...
        $tableNameMapper->getTableName('pimee_team_work_assistant.product_category')
            ->willReturn('pimee_team_work_assistant_product_category');

        $connection->delete('pimee_team_work_assistant_product_category', [
            'product_id' => 'fdsqf121s3s'
        ])->shouldBeCalled();

        $connection->insert('pimee_team_work_assistant_product_category', [
            'product_id' => 'fdsqf121s3s',
            'category_id' => 40,
        ])->shouldBeCalled();
