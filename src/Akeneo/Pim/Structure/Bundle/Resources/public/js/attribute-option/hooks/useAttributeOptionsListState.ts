...
export const useAttributeOptionsListState = ():AttributeOptionsListState => {
    ...
    const addExtraData = useCallback((code: string, extra: ExtraData) => {
    ...
    const removeExtraData = useCallback((code: string) => {
        if (!getAttributeOption(code)) {
            return;
        }
    ...
    useEffect(() => {
        ...
        window.dispatchEvent(new CustomEvent<AttributeOptionsListStateEvent>(ATTRIBUTE_OPTIONS_LIST_LOADED, {
            detail: {
                attributeOptions,
                getAttributeOption,
                addExtraData,
