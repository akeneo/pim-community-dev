...
const useOffsetAvailableSources = (debouncedSearchValue: string, type: string, shouldFetch = true) => {
  ...
  const sourcesFetcher = useAvailableSourcesFetcher(debouncedSearchValue, userContext.get('catalogLocale'), type);
