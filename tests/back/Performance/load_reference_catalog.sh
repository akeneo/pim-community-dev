...
docker-compose exec -T fpm bin/console pim:user:create -e behat --admin -n -- admin admin test@example.com John Doe en_US
...
docker pull akeneo/data-generator:1.0
...
docker run \
  ...
  akeneo/data-generator:1.0 akeneo:api:generate-catalog --with-products --check-minimal-install product_api_catalog.yml
