...
Feature: Export published products in XLSX
  ...
  Scenario: Successfully export published products into several files
    ...
    Then exported xlsx file 1 of "xlsx_clothing_mobile_published_product_export" should contain:
      | sku          | categories                | datasheet | description-de_DE-mobile         | description-en_US-mobile      | description-fr_FR-mobile | enabled | family  | gallery | groups | handmade | length | main_color | manufacturer | name-de_DE    | name-en_US   | name-fr_FR   | number_in_stock-mobile | price-EUR | price-USD | rating | release_date-mobile | secondary_color | side_view | size | top_view | weather_conditions |
