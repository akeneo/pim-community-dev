...
const DeleteSupplier = ({identifier, onSupplierDeleted, onCloseModal}: Props) => {
    ...
    const deleteRoute = useRoute('supplier_portal_retailer_supplier_delete', {identifier});

    const deleteSupplier = async () => {
        const response = await fetch(deleteRoute, {method: 'DELETE'});
        if (!response.ok) {
            notify(NotificationLevel.ERROR, translate('supplier_portal.supplier.supplier_delete.modal.unknown_error'));
            return;
        }

        notify(NotificationLevel.SUCCESS, translate('supplier_portal.supplier.supplier_delete.sucess_message'));
        onSupplierDeleted();
    };
    ...
    return (
        <Modal onClose={onCloseModal} closeTitle={translate('pim_common.close')} illustration={<DeleteIllustration />}>
            <Modal.SectionTitle color="brand">
                {translate('supplier_portal.supplier.supplier_delete.modal.title')}
            </Modal.SectionTitle>
