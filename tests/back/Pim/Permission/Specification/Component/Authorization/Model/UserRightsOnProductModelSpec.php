...
class UserRightsOnProductModelSpec extends ObjectBehavior
{
    ...
    {
        $this->beConstructedWith('test', 1, 2, 3, 1, 3);
        $this->shouldBeAnInstanceOf(UserRightsOnProductModel::class);
    }

    function it_is_a_product_model_that_the_user_can_view_when_the_product_model_is_categorized_in_at_least_one_category_viewable_by_the_user(): void {
        $this->beConstructedWith('test', 1, 0, 0, 1, 3);
        $this->isProductModelViewable()->shouldReturn(true);
    }

    function it_is_a_product_model_that_the_user_can_view_when_the_product_model_is_not_categorized(): void {
        $this->beConstructedWith('test', 1, 0, 0, 0, 0);
        $this->isProductModelViewable()->shouldReturn(true);
    }
    ...
    function it_is_a_product_model_that_the_user_can_edit_when_the_product_model_is_not_categorized(): void {
        $this->beConstructedWith('test', 1, 0, 0, 1, 0);
        $this->isProductModelEditable()->shouldReturn(true);
    }
    ...
    function it_is_feasible_for_the_user_to_apply_a_draft_on_the_product_model_when_the_product_model_is_categorized_in_at_least_one_category_editable_and_no_category_owned_by_the_user(): void {
        $this->beConstructedWith('test', 1, 1, 0, 1, 1);
