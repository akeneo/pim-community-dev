 ...
 */
class SaveProductTwoWayAssociations implements EventSubscriberInterface
{
    /**
     * @var Connection
     */
        ...
        foreach ($product->getAssociations() as $association) {
            if ($association->getAssociationType()->isBidirectional()) {
                $this->connection->transactional(function() use ($product, $association) {
                    $this->saveInvertedProductAndProductAssociation($association, $product);
                    $this->saveInvertedProductAndProductModelAssociation($association, $product);
                });
            }
        }
    }

    private function saveInvertedProductAndProductAssociation(
    ...
    private function saveInvertedProductAndProductModelAssociation(
        AssociationInterface $association,
        ProductInterface $product
    ) {
        $this->removeInvertedProductModelAssociationsDeleted($association, $product);
        $this->saveBidirectionalProductModelAssociation($association, $product->getId());
    }
...
SQL;
    ...
    private function saveProductModelAssociation($newAssociationId, $ownerProductId)
    {
        $insertProductAssociation = <<<SQL
