...
class FieldFilterHelper
{
    ...
    /**
     * Get the field with its property
     * @param string $field
     * @param string $default
     *
     * @return string
    ...
    {
        if (!is_array($value)) {
            throw InvalidArgumentException::arrayExpected($field, 'filter', $filter);
        }
    }
    ...
    public static function checkIdentifier($field, $value, $filter)
    {
        ...
        if (static::hasProperty($field) &&
            ...
            throw InvalidArgumentException::stringExpected($field, 'filter', $filter);
        }
    }
}
