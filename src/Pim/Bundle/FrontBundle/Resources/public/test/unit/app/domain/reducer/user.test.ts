...
describe('>>>REDUCER --- user', () => {
  ...
  test('handle switch on new locale', () => {
    expect(reducer(undefined, {type: 'LOCALE_CHANGED', target: 'copyLocale', locale: 'fr_FR'})).toEqual({
      copyLocale: 'fr_FR',
    });
  });
  ...
  test('handle switch on existing target locale', () => {
    expect(
      reducer({catalogLocale: 'fr_FR'}, {type: 'LOCALE_CHANGED', target: 'catalogLocale', locale: 'en_US'})
  ...
  test('handle switch on existing wrong value', () => {
    expect(reducer({}, {type: 'CHANNEL_CHANGED', target: 'catalogChannel', locale: 'ecommerce'})).toEqual({});
  });
});
