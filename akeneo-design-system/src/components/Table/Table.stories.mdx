...
<Meta
  ...
  component={Table}
  subcomponents={{'Table.Header': Table.Header, 'Table.HeaderCell': Table.HeaderCell, 'Table.Body': Table.Body,' Table.Row': Table.Row, 'Table.Cell': Table.Cell }}
  argTypes={{
                            ...
                            sortedData.map((row) => {
                                return (
                                    <Table.Row key={row.name}>
                                        <Table.Cell primary={true}>{row.name}</Table.Cell>
...
## Test Clickable selectable lines
<Canvas>
    <Story name="Selectable Clickable lines">
        {args => {
            const [selectedLines, setSelectedLines] = useState([]);
            const handleToggleSelected = (lineId) => {
                if (selectedLines.indexOf(lineId) === -1) {
