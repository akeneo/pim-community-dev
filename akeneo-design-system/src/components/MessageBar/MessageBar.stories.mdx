...
<Meta
  ...
  argTypes={{
    level: {control: {type: 'select', options: ['info', 'success', 'warning', 'danger']}},
...
We recommend that messages automatically disappear after 3 seconds for success, 5s for warning and 8s for danger.
Manual closing instead timer if there is a link in the message.
    ...
    {args => {
      return (
        <>
          ...
          <MessageBar {...args} title="Danger" level="danger" icon={<Icons.DangerIcon />} />
        </>
      );
    }}
  </Story>
...
<Canvas>
  <Story name="Content">
    {args => {
      ...
      return (
        <>
          <MessageBar {...args} level="success" title="This one has a title only" icon={<Icons.TagIcon />} />
          <MessageBar {...args} title="This one has a Link" icon={<Icons.LinkIcon />}>
            The MessageBar can also be combined with the Link component. <Link>Take a look here</Link>
          </MessageBar>
        </>
      );
    }}
