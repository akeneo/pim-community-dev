...
class MaxValueUpdaterSpec extends ObjectBehavior
{
    ...
    function it_only_supports_edit_max_value_command_for_number_attributes(
        ...
        $editMaxCommand = new EditMaxCommand('number', '10');
    ...
    {
        $editMax = new EditMaxCommand( 'max', '10');

        $this->__invoke($numberAttribute, $editMax)->shouldReturn($numberAttribute);

        $numberAttribute->setMaxValue(AttributeMaxValue::fromString('10'))->shouldBeCalled();
    }

    function it_unsets_the_max_value_of_an_attribute(NumberAttribute $numberAttribute)
    {
        $editMax = new EditMaxCommand( 'max', null);

        $this->__invoke($numberAttribute, $editMax)->shouldReturn($numberAttribute);

        $numberAttribute->setMaxValue(AttributeMaxValue::noMaximum())->shouldBeCalled();
    }
    ...
    {
        $supportedCommand = new EditMaxCommand( 'max', null);
