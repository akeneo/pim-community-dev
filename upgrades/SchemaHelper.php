...
class SchemaHelper
{
    ...
    protected $productResources = [
        'product', 'product_media', 'published_product', 'published_product_media'
    ];
    ...
    public function getTableOrCollection($resource)
    {
        ...
        if ($this->upgradeHelper->areProductsStoredInMongo() && in_array($resource, $this->productResources)) {
            return $this->getTableOrCollectionForMongoProducts();
        }
     ...
     * @return string
     */
    private function getTableOrCollectionForMongoProducts()
    {
        $class = $this->container->getParameter($this->classMapping['product']);
