...
test('it does not display the channel, locale and operator selectors depending if a quality score is not selected', () => {
  ...
  expect(screen.queryByText('akeneo.syndication.filters.quality_score.locales.label')).not.toBeInTheDocument();
});
...
test('it displays the channel, locale and operator selectors depending if a quality score selected', () => {
  ...
  expect(screen.getByText('akeneo.syndication.filters.quality_score.locales.label')).toBeInTheDocument();
});
...
test('it can switch locales', () => {
  ...
  userEvent.click(screen.getByLabelText('akeneo.syndication.filters.quality_score.locales.label'));
...
test('when the user resets the quality score it removes the context and the operator', () => {
  ...
  userEvent.click(screen.getByText('pim_enrich.export.product.filter.quality-score.title'));
  userEvent.click(screen.getByTitle('akeneo.syndication.filters.quality_score.score.remove'));
