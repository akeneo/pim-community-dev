...
namespace Akeneo\Platform\Syndication\Infrastructure\Connector\Reader\Database;
    ...
    TrackableItemReaderInterface
{
    ...
    {
        $filters = [];
        if ($this->getStepExecution()->getJobParameters()->has('catalogProjections')) {
            $filters = $this->getStepExecution()->getJobParameters()->get('catalogProjections')[0]['filters'] ?? [];
        }
