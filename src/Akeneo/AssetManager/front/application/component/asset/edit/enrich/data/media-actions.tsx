...
export const DownloadAction = ({data, attribute, label, size, color}: ActionProps) => {
  if ((isMediaLinkAttribute(attribute) && MediaTypes.youtube === attribute.media_type) || isDataEmpty(data))
    return null;
...
export const CopyUrlAction = ({data, attribute, label, size, color}: ActionProps) => {
  ...
  const url = MediaTypes.youtube === attribute.media_type ? getYouTubeWatchUrl(data) : getMediaLinkUrl(data, attribute);
