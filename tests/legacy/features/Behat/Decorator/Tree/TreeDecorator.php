...
class TreeDecorator extends ElementDecorator
{
    ...
    public function findNodeInTree(string $nodeName): TreeDecorator
    {
        return $this->spin(
            function () use ($nodeName) {
                $trees = $this->element->findAll('css', 'li[role=treeitem]');
            ...
            sprintf(
                ...
                }, $this->element->findAll('css', 'li[role=treeitem]')))
            )
        );
    }
    ...
    {
        $checkbox = $this->element->find('css', 'div[role=checkbox]');
        if (null === $checkbox) {
            throw new ElementNotFoundException($this->element->getDriver(), 'div[role=checkbox]');
        }
